...
describe('listModels', () => {
  it('returns model name list from /models/', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/models/', expect.any(Object));
  });
});
...
describe('getThreads', () => {
  it('returns thread list from /threads/', async () => {
    ...
    expect(threads[0].metadata?.title).toBe('test');
    expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/threads/', expect.any(Object));
  });
});
