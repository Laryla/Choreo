...
把你散落在各处的信息汇入一处，让 agent 在干活时能主动检索：

- **四类内容**：个人笔记 / 文档、工作上下文与偏好、外部系统数据（飞书、GitHub、日历等自动拉取）、工作流产物沉淀（每次任务的输出自动归档）。
- **RAG 检索**：agent 执行任务前先检索知识库，把相关上下文注入 prompt，生成结果更贴近你的实际情况。
- **主动入库**：调度器定期从外部系统同步数据，无需手动导入。
...
- [ ] **个人知识库**：笔记 / 外部系统数据 / 工作流结果自动归档 + RAG 检索（agent 干活前主动查）
- [ ] **AI 持久记忆**：向量化记忆，跨会话长期记住偏好与工作上下文
