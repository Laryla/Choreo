...
- Node.js >= 18（前端）
- Python >= 3.10（后端）
- 已安装并登录 [`gh` CLI](https://cli.github.com/)
...
# 后端
cd backend && pip install -e . && cp .env.example .env   # 填好 .env
...
```ini
# LLM（OpenAI 兼容，可接 deepseek）
OPENAI_API_KEY=sk-xxxx
OPENAI_BASE_URL=https://api.deepseek.com/v1
CHOREO_MODEL=deepseek-chat

# 记忆与技能存储目录
CHOREO_MEMORY_DIR=./.choreo/memory
CHOREO_SKILLS_DIR=./.choreo/skills
