...
cd backend
uv sync                     # 安装依赖
uv run uvicorn choreo.gateway.app:app --reload --port 8000
```
...
不需要沙箱时，改为 `active_sandbox: local-dev` 即可使用本地文件系统。
