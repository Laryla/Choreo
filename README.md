...
用得越久，它越强。支持 **Web UI** 和 **命令行 CLI** 两种使用方式。
...
CLI 连接同一套后端，无需额外配置，支持流式输出、思考块折叠、HITL 确认、`/model` 切换模型等功能。
...
choreo             # 启动交互式 REPL
choreo run "帮我列出当前目录"  # 单次运行后退出
```

首次运行会引导配置后端地址和主题色。输入 `/help` 查看所有命令，`/` 直接浏览命令列表。

> 也可通过环境变量指定后端：`CHOREO_API_URL=http://localhost:8009 choreo`
...
│  · 对话输入            │
│  · 脚本审阅 / 确认     │        ┌─────────────────────────────────────────┐
│  · 技能库管理 & pin    │  HTTP  │         后端 (FastAPI + LangChain)        │
│  · 记忆查看 / 编辑     │ ─────▶ │                                          │
│  · 运行历史 & 产物     │ ◀───── │  create_agent 智能体 (ReAct 循环)         │
└───────────────────────┘  SSE  │   ├─ tools: 读git / 生成脚本 / 执行 / 通知 │
                                 │   ├─ middleware: 人在回路 + 调用上限 + 压缩  │
┌───────────────────────┐        │   └─ checkpointer: 中断/恢复               │
│   CLI (TypeScript)    │  HTTP  │                                          │
│  · 流式对话 REPL       │ ─────▶ │  🧠 记忆: USER.md + MEMORY.md + 历史检索   │
│  · 思考块折叠          │ ◀───── │  📚 知识库: raw/ → LLM 编译 → wiki/ + 图谱  │
│  · HITL 终端确认      │  SSE  │  📈 技能: 自主沉淀 / 使用中修补 / 策展      │
│  · /model 切换模型    │        │  ⏰ 调度: APScheduler 定时跑               │
│  · 斜杠命令选择器      │        │  📦 沙箱: 隔离执行生成的脚本               │
└───────────────────────┘        └─────────────────────────────────────────┘
                                              ▲
                                 ┌────────────┴────────────┐
                                 │  💬 聊天平台 (可插拔)     │
...
- [x] **命令行 CLI（choreo）**：Claude Code 风格 REPL，流式输出 + 思考块折叠 + HITL 终端确认 + 斜杠命令选择器 + 启动状态面板
- [x] MCP 工具集成：可视化配置 MCP Server 和工具审批
- [x] 定时任务：无人值守 agent 自动运行，结果推送飞书（全局 Bot，无需配置 Webhook）
- [x] 上下文压缩：长对话（≥60条消息或≥60k token）自动摘要，保持模型注意力集中
- [x] **个人知识库（LLM Wiki）**：LLM 把原始资料「编译」成结构化 wiki 页面 + `[[wiki-link]]`，agent 工具 grep 检索；工作流结果自动归档进 raw/
- [x] **知识图谱可视化**：解析 `[[wikilinks]]` 自动构建图谱，D3 力导向图 + 拖拽平移 + hover 实虚线高亮
- [ ] **可插拔知识来源适配器**：飞书知识库、语雀等一行配置接入，调度器定时拉取写入 raw/
- [ ] **自动活动日志**：每周自动汇总 commit / 飞书消息 / 任务输出，LLM 编译成「这周干了什么 + 我在关注什么」wiki 页面
- [ ] **AI 持久记忆**：跨会话长期记住偏好与工作上下文，对话后自动提取关键事实
- [ ] **多 agent 聊天室**：人类 + 多 AI 在同一聊天室协作办公，目标明确、分工清晰
- [ ] 技能市场（社区共享与一键安装）
- [ ] GitHub Webhook 触发
- [ ] 可插拔记忆 provider
- [ ] 飞书 HITL 交互卡片（按钮确认）
