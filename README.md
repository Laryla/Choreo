...
- **知识图谱自动生成**：解析 wiki 页面里的 `[[实体链接]]` 即得图谱，前端可视化展示实体之间的关系网络。
- **自动活动日志**：每周自动汇总你的 commit、飞书消息、定时任务输出，LLM 编译成「这周干了什么 + 我在关注什么」——你正常工作，知识库自动成长。
- **可插拔来源**：飞书知识库、语雀等一行配置接入，调度器定时同步。
...
│  · 技能库管理 & pin    │  SSE   │   ├─ middleware: 人在回路 + 调用上限 + 上下文压缩 │
│  · 记忆查看 / 编辑     │        │   └─ checkpointer: 中断/恢复               │
│  · 运行历史 & 产物     │        │                                          │
└───────────────────────┘        │  🧠 记忆: USER.md + MEMORY.md + 历史检索   │
                                 │  📚 知识库: 笔记/外部数据/产物归档 + RAG    │
                                 │  📈 技能: 自主沉淀 / 使用中修补 / 策展      │
                                 │  ⏰ 调度: APScheduler 定时跑               │
                                 │  📦 沙箱: 隔离执行生成的脚本               │
                                 └─────────────────────────────────────────┘
...
- [ ] **个人知识库（LLM Wiki）**：LLM 把原始资料「编译」成结构化 wiki 页面 + `[[wiki-link]]` 知识图谱，agent 用 grep 检索而非向量检索；工作流结果自动归档进 raw/
- [ ] **知识图谱可视化**：解析 wiki 页面中的 `[[实体链接]]` 自动构建图谱，前端力导向图展示实体关系
